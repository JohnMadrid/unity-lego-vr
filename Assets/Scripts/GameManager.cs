...
public class GameManager : MonoBehaviour
{
    ...
    public int trialNumber = 1; // change in each trial when fully finished !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    void SaveResponseToCSV(string response)
    {
        ...
        if (questionPhase == 1)
        {
            if (string.IsNullOrEmpty(csvPathQ1))
            {
                ...
                csvPathQ1 = Path.Combine(questionLogPath, $"{participantCode}_MentalLoadResponses_Trial{trialNumber}_{now:yyyy-MM-dd-HH-mm}.csv");
            }
        ...
        else if (questionPhase == 2)
        {
            ...
            {
                DateTime now = DateTime.Now;
                csvPathQ2 = Path.Combine(questionLogPath, $"{participantCode}_SuccessResponses_Trial{trialNumber}_{now:yyyy-MM-dd-HH-mm}.csv");
            }
        ...
        {
            // Save complexity response to a separate CSV file
            string complexityCsvPath = Path.Combine(questionLogPath, $"{participantCode}_ComplexityResponses_Trial{trialNumber}_{DateTime.Now:yyyy-MM-dd-HH-mm}.csv");
            bool fileExists = File.Exists(complexityCsvPath);
            using (var writer = new StreamWriter(complexityCsvPath, append: true))
            {
                if (!fileExists)
                    writer.WriteLine("ParticipantCode,TrialNumber,ItemNumber,Response");
