...
public class BrickSnapController : MonoBehaviour
{
    ...
    {
        GameObject brick = gameObject.GetComponentInParent<XRBaseInteractable>().gameObject;

        // Deactivate collider triggers for grabbed brick
        Collider[] colliders = GetComponentsInChildren<Collider>()
            .Where(c => c.gameObject.CompareTag("SnapPoint") && c.isTrigger)
            .ToArray();
        foreach (var collider in colliders) 
        {
            collider.enabled = true;
            collider.isTrigger = false;
            collider.gameObject.GetComponentInChildren<SnapPointCollider>().isGrabbed = true;
        }
        Debug.Log(brick.name + $" - OnSelectEntered: Colliders '{string.Join(", ", colliders.Select(sp => sp.name))}' found - DISABLED trigger on them and SET to grabbed" );
    }
    ...
    public void OnSelectExited(SelectExitEventArgs args)
    {
        ...
        // SnapPointCollider[] matchingSnapPoints = brick.GetComponentsInChildren<SnapPointCollider>()
        //     .Where(s => s.matchingSnapPointCollider != null)
        //     .ToArray();
        // if (matchingSnapPoints.Length > 0)
        // {
        //     Debug.Log(brick.name + $" - OnSelectEntered: Matching SnapPoints present: {string.Join(", ", matchingSnapPoints.Select(sp => sp.name))}" );
        //     foreach (var snapPoint in matchingSnapPoints)
        //     {
        //         snapPoint.SnapObjects();
        //     }
        // }

        // Activate collider triggers for un-grabbed brick
        Collider[] colliders = GetComponentsInChildren<Collider>()
            .Where(c => c.gameObject.CompareTag("SnapPoint") && !c.isTrigger)
            .ToArray();
        foreach (var collider in colliders) 
        {
            ...
            collider.gameObject.GetComponentInChildren<SnapPointCollider>().isGrabbed = false;
        }
        Debug.Log(brick.name + $" - OnSelectEntered: Colliders '{string.Join(", ", colliders.Select(sp => sp.name))}' found - ENABLED trigger on them and UNSET grabbed" );
