...
{
    const float UNIT = 0.4f;           // one “stud” unit
    const float BRICK_HEIGHT = 0.8f * UNIT; // height of a standard brick
    const float PLATE_HEIGHT = BRICK_HEIGHT / 3f; // height of a plate/board
    const float STUD_HEIGHT = 0.2f * UNIT;  // height of a stud/tube
    const float STUD_RADIUS = 0.15f * UNIT;  // radius of a stud/tube
    const int Fidelity = 8; // Segments for studs and holes. Higher is smoother.
    const int FACE_GRID_RESOLUTION = 4; // Lower value for a simpler face mesh.
 
    [MenuItem("GameObject/Create Lego Bricks/Generate 1×1 Brick", false, 10)]
    public static void Create1x1() => CreateBrick(1, 1);
 
    [MenuItem("GameObject/Create Lego Bricks/Generate 1×2 Brick", false, 11)]
    public static void Create1x2() => CreateBrick(1, 2);
 
    [MenuItem("GameObject/Create Lego Bricks/Generate 2×2 Brick", false, 12)]
    public static void Create2x2() => CreateBrick(2, 2);
 
    [MenuItem("GameObject/Create Lego Bricks/Generate 3×2 Brick", false, 13)]
    public static void Create3x2() => CreateBrick(3, 2);
 
    [MenuItem("GameObject/Create Lego Bricks/Generate 1×3 Brick", false, 14)]
    public static void Create1x3() => CreateBrick(1, 3);
 
    [MenuItem("GameObject/Create Lego Bricks/Generate 8x8 Baseplate", false, 100)]
    public static void Create8x8Baseplate() => CreateBoard(8, 8);
 
    private static void CreateBrick(int width, int length)
    {
        // Generate random color
        Color randomColor = GetRandomLegoColor();
        string colorName = GetColorName(randomColor);
        
        // 1) Parent container
        var brick = new GameObject($"Lego_{width}x{length}_{colorName}");
        brick.transform.localScale = new Vector3(0.13f, 0.13f, 0.13f);
        ...
        material.SetFloat("_Smoothness", 0.3f); // High smoothness for a shiny finish
        ...
        // 11) The mesh is already created with Z pointing down, so no rotation is needed.
 
        // 12) Select new brick
        Selection.activeGameObject = brick;
    }
    
    private static void CreateBoard(int width, int length)
    {
        // 1) Parent container
        var board = new GameObject($"Lego_Board_{width}x{length}");
        board.transform.localScale = new Vector3(0.15f, 0.15f, 0.15f);
    ...
    private static Color GetRandomLegoColor()
    {
        Color[] legoColors = new Color[]
        {
            Color.red,
            Color.blue,
            Color.green,
            ...
            new Color(0.8f, 0.6f, 0.2f), // Brown
    ...
    private static string GetColorName(Color color)
    {
        ...
        if (color == new Color(0f, 1f, 1f)) return "Cyan";
        if (color == new Color(0.5f, 0f, 0.5f)) return "Purple";
        if (color == new Color(0.8f, 0.6f, 0.2f)) return "Brown";
        if (color == new Color(1f, 0.8f, 0.8f)) return "Pink";
    ...
    private static Mesh CreateBrickMesh(int width, int length)
    {
        ...
        // Part 2: Top Face (with holes for studs)
        var studPositions = GetStudPositions(width, length);
        var topFaceMesh = CreateFaceWithHoles(width, length, studPositions, STUD_RADIUS, false);
        var topTransform = Matrix4x4.Translate(new Vector3(0, 0, -halfHeight));
        combine.Add(new CombineInstance { mesh = topFaceMesh, transform = topTransform });

        // Part 3: Bottom Face (with holes for female studs)
        var bottomFaceMesh = CreateFaceWithHoles(width, length, studPositions, STUD_RADIUS * 1.05f, true); // Inverted
        var bottomTransform = Matrix4x4.Translate(new Vector3(0, 0, halfHeight));
        combine.Add(new CombineInstance { mesh = bottomFaceMesh, transform = bottomTransform });
    ...
    private static Mesh CreateFaceWithHoles(int width, int length, Vector2[] holePositions, float holeRadius, bool invert)
    {
        ...
        foreach (var holeCenter in holePositions)
        {
            var holePath = new List<Vector2>();
            for (int i = 0; i < Fidelity; i++)
            {
                float angle = i * 2f * Mathf.PI / Fidelity;
                float x = holeCenter.x + Mathf.Cos(angle) * holeRadius;
                float y = holeCenter.y + Mathf.Sin(angle) * holeRadius;
                holePath.Add(new Vector2(x, y));
            }
            holes.Add(holePath);
        }

        var triangulator = new Triangulator(boundary.ToArray(), holes.Select(h => h.ToArray()).ToArray());
        int[] newTriangles = triangulator.Triangulate();

        foreach(var p in triangulator.Points)
